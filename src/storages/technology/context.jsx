...
export const TechnologyContextProvider = ({ children, defaultValues }) => {
    ...
    return (
        <TechnologyContext.Provider value={{ state, setIsLoading, setSelected, fetchTopStories }}>
            {children}
...
const useTechnologyContext = () => {
    const { state, setIsLoading, setSelected, fetchTopStories } = useContext(TechnologyContext);

    return { setIsLoading, setSelected, fetchTopStories, ...state };
};

export default useTechnologyContext;
